PamMain() // Starts when map is loaded.
{
	...
	if(!isdefined(game["runonce"]))
	{
		...
		{
		case "twl":
			thread maps\mp\gametypes\rules\_twl_bas_rules::Rules();
			break;
		case "na_comp":
			thread maps\mp\gametypes\rules\_na_comp_bas_rules::Rules();
			break;
		case "na_comp_pub":
			thread maps\mp\gametypes\rules\_na_comp_bas_rules::Rules();
			break;
		case "lan":
			thread maps\mp\gametypes\rules\_lan_bas_rules::Rules();
			break;

		default:
			thread maps\mp\gametypes\rules\_public_bas_rules::Rules();
			setCvar("pam_mode", "pub");
			break;
		}
...
// ----------------------------------------------------------------------------------
//	menu_spawn  //////Edited by:|OCG|=gLiTcH=//////
//
...
///////////////////////////////////////////////////////////////////////////////////		
                ...
                if(!level.didexist[self.pers["team"]] && !level.roundended) //Edited By:|OCG|=gLiTcH=

//Above line was.....else if(!level.didexist[self.pers["team"]] && !level.roundended)
...
RestartMap( )
{
	...
	if (!isdefined(game["readyupdone"]))
	{
		game["readyupdone"] = true;
		...
		if (game["mode"] == "match")
		{
			...
			Create_HUD_PlayersReady();
			skip_gmi_announcement = true;
		}
	}
	
	if (!isdefined(skip_gmi_announcement))
