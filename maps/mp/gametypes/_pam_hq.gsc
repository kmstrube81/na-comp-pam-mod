PamMain()
{
	...
	if(!isdefined(game["runonce"]))
	{
		...
		{
		case "twl":
			thread maps\mp\gametypes\rules\_twl_hq_rules::Rules();
			break;
		case "na_comp":
			thread maps\mp\gametypes\rules\_na_comp_hq_rules::Rules();
			break;
		case "na_comp_pub":
			thread maps\mp\gametypes\rules\_na_comp_pub_hq_rules::Rules();
			break;
		case "lan":
			thread maps\mp\gametypes\rules\_lan_hq_rules::Rules();
			break;

		default:
			thread maps\mp\gametypes\rules\_public_hq_rules::Rules();
			setCvar("pam_mode", "pub");
			break;
		}
...
Check_Match_Start()
{
	...
	while (!level.exist["teams"])
	{
		...
		if(getCvarInt("scr_debug_hq") != 1) //So 1 person can play.
		{
